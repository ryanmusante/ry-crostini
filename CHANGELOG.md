...
- README Table of Contents converted from nested bullet list to a numbered two-column table (`#` / `Section`) with 13 top-level entries and 10 sub-entries under Gaming Reference (`12.1` through `12.10`). Scannable at-a-glance and gives readers a stable numeric handle when referencing sections in bug reports or deep-research prompts. Anchors unchanged; validated all 25 against actual `##`/`###` header slugs post-conversion.
